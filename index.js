...
// ANSWER: no because (and i put the wrong one first) it would normally have been O(n) because of the single for loop, but we have it stopping from running after 4 times. so it is the best we can do.
...
// ANSWER: i do not think so, since the space is going to be primarily determined by the size of the numbers array. aka not a fixed amount of data.
...
// ANSWER: i believe this is the same as the one from question 14 because the space will depend on the size of the str that is going to be passed into either of them.